...
interface BarGraphProps {
  previousMonth: {
    essentials: { spent: number; expenseLimit: number };
    wants: { spent: number; expenseLimit: number };
  };
  currentMonth: {
    essentials: { spent: number; expenseLimit: number };
    wants: { spent: number; expenseLimit: number };
...
const ExpenditureBarGraph: React.FC<BarGraphProps> = ({
  previousMonth,
  currentMonth,
  budgetLimit,
  currentMonthDate,
  ...
  const renderStackedBar = (
    ...
    const essentialsHeight =
      Math.min(essentialsSpent, expenseLimit) * scaleFactor;
    const wantsHeight =
      Math.min(wantsSpent, Math.max(0, expenseLimit - essentialsSpent)) *
      scaleFactor;
    ...
    const maxHeight = Math.max(
      essentialsHeight + wantsHeight + exceededHeight,
      0
    );
    const tooltipPosY =
      graphHeight - maxHeight - tooltipHeight - tooltipPadding - 60 - overlap;
    ...
    const renderBarSegment = (
      ...
      return (
        <rect
          x={x}
          y={graphHeight - 50 - height - offsetY}
          width="46"
          height={height}
          fill={color}
          rx="10"
    ...
    return (
      <g>
        ...
        {exceededHeight > 0 &&
          renderBarSegment(
            essentialsHeight + wantsHeight - overlap,
            exceededHeight + overlap,
            "#f99e36"
          )}
        {/* Wants segment of the bar */}
        {wantsHeight > 0 &&
          renderBarSegment(
            essentialsHeight - overlap,
            wantsHeight + overlap,
            "#9DB1C6"
          )}
        ...
        {essentialsHeight > 0 &&
          renderBarSegment(
            0,
            essentialsHeight + (wantsHeight > 0 ? overlap : 0),
            "url(#essentialsGradient)"
          )}
        {showTooltip && totalSpent > 0 ? (
          <g style={{ zIndex: 2 }}>
            <rect
              x={x + 1}
              y={tooltipPosY + yOffset}
              width="47"
            ...
            <text
              x={x + 23}
              y={tooltipPosY + tooltipHeight / 2 + 3 + yOffset}
              textAnchor="middle"
              fill="#f6f6f7"
              fontSize="10"
              fontFamily="Poppins"
            ...
            <text
              x={x + 20}
              y={tooltipPosY + tooltipHeight + 13 + yOffset}
              textAnchor="middle"
              fontSize="10"
              fontFamily="Poppins"
            >
  ...
  const renderLimitLine = (value: number, label: string) => (
    <g>
      <line
        x1="34"
        y1={graphHeight - 50 - value * scaleFactor}
        x2={graphWidth}
        y2={graphHeight - 50 - value * scaleFactor}
        stroke="black"
        strokeDasharray="4"
      />
      <text
        x={2}
        y={graphHeight - 50 - value * scaleFactor + 5}
        textAnchor="start"
        fontSize="11"
        fill="#878787"
        fontFamily="Poppins"
      ...
      >
        <defs>
          ...
          >
            <stop
              offset="0%"
              style={{ stopColor: "#144CBC", stopOpacity: 1 }}
            />
            <stop
              offset="100%"
              style={{ stopColor: "#0131A1", stopOpacity: 1 }}
            />
          </linearGradient>
        </defs>
        ...
        {renderStackedBar(
          0.2,
          previousMonth.essentials.spent,
          previousMonth.wants.spent,
          false
        )}
        ...
        {renderStackedBar(
          0.39,
          currentMonth.essentials.spent,
          currentMonth.wants.spent,
          true
        )}
        ...
        {/* Month Labels */}
        {previousMonth.essentials.spent + previousMonth.wants.spent > 0 ? (
          <text
            x={graphWidth * 0.2 + 22}
            y={graphHeight - 35}
            textAnchor="middle"
...
export const ChangeIndicator: React.FC<Props> = ({
  previousMonth,
  currentMonth,
  graphWidth,
  graphHeight,
  ...
  const totalPrevMonth =
    previousMonth.essentials.spent + previousMonth.wants.spent;
  const totalCurrMonth =
    currentMonth.essentials.spent + currentMonth.wants.spent;
  const changePercentage =
    ((totalCurrMonth - totalPrevMonth) / totalPrevMonth) * 100;
    ...
    <svg>
      ...
      >
        {isIncrease ? (
          ...
          <FiArrowDownRight color="#565656" strokeWidth="1" />
        )}
      </g>
