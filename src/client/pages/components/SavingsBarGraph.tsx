...
const SavingsBarGraph: React.FC<BarGraphProps> = ({
  ...
  const renderBar = (
    ...
    const tooltipPosY =
      graphHeight - savingsHeight - tooltipHeight - tooltipPadding - 60;
    ...
    return (
      <g>
        ...
        <rect
          x={x}
          y={graphHeight - 50 - savingsHeight}
          width="46"
        ...
        {showTooltip && (
          <>
            ...
            <text
              x={x + 23}
              y={tooltipPosY + tooltipHeight / 2 + 3}
              textAnchor="middle"
              fill="#f6f6f7"
              fontSize="10"
              fontFamily="Poppins"
            ...
            <text
              x={x + 20}
              y={tooltipPosY + tooltipHeight + 13}
              textAnchor="middle"
              fontSize="10"
              fontFamily="Poppins"
            >{`+${savings.toLocaleString("en-US")}`}</text>
          </>
        )}
      </g>
    );
  ...
  const renderChangeContainer = () => {
    ...
    const changePercentage = predictHighestPossibleGrowth(
      previousMonthSavings,
      currentMonthSavings
    );
