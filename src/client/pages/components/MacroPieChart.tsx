...
const MacroPieChart: React.FC<MacroPieChartProps> = ({ dimensions, doughnutThickness, values }) => {
    ...
    const items = [
        { percentage: (unallocated / total) * 100, color: '#cfddea' },
        { percentage: (savings / total) * 100, color: '#3da134' },
        { percentage: (wants / total) * 100, color: '#c69155' },
        { percentage: (essentials / total) * 100, color: '#0c8eb0' },
    ];
        ...
        <svg height={dimensions} width={dimensions}>
            <g transform={`translate(${radius}, ${radius})`}>
                {items.map((item, index) => {
                    ...
                    const endX = normalizedRadius * Math.cos(2 * Math.PI * (cumulativePercentage + item.percentage) / 100);
                    const endY = normalizedRadius * Math.sin(2 * Math.PI * (cumulativePercentage + item.percentage) / 100);
