...
const InsightsExpenditureChart: React.FC<InsightsExpenditureChartProps> = ({ currencySymbol, essentialsArray, wantsArray }) => {
    const [options, setOptions] = useState<ApexOptions>({
        ...
        tooltip: {
            custom: ({ series, seriesIndex, dataPointIndex, w }) => {
                const formattedDate = w.config.series[0].data[dataPointIndex]?.x.length !== 7 ?
                ...
                return (
                    `<div style="padding: 10px; background-color: #f4f9fb; border-radius: 8px; font-size: 14px;" class="custom-tooltip">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                        <div style="display: flex; align-items: center;">
                            ...
                            <span style="color: #0131A1;">Essentials:</span>
                        </div>
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                        <div style="display: flex; align-items: center;">
                            ...
                            <span style="color: #9DB1C6;">Wants:</span>
                        </div>
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div style="display: flex; align-items: center;">
                            ...
                            <span style="color: #101010;">Total:</span>
                        </div>
    ...
    useEffect(() => {
        const [updatedEssentialsArray, updatedWantsArray] = alignDataArrays(essentialsArray, wantsArray);
...
const calculateTotalSpend = (essentials: { x: any; y: any; }[], wants: { x: any; y: any; }[]) => {
...
function alignDataArrays(essentials: DataPoint[], wants: DataPoint[]): [DataPoint[], DataPoint[]] {
...
function alignMonthDataArrays(essentials: DataPoint[], wants: DataPoint[]): [DataPoint[], DataPoint[]] {
    ...
    // Align arrays
    const alignArray = (array: DataPoint[], earliest: number, latest: number): DataPoint[] => {
...
function alignDayDataArrays(essentials: DataPoint[], wants: DataPoint[]): [DataPoint[], DataPoint[]] {
    ...
    const alignArray = (array: DataPoint[], start: Date, end: Date, monthly: boolean): DataPoint[] => {
        ...
        for (let currentDate = new Date(start); currentDate <= end; currentDate = incrementDate(currentDate, monthly)) {
            const dateKey = monthly ? currentDate.toISOString().substring(0, 7) : currentDate.toISOString().substring(0, 10);
            if (dateMap.has(dateKey)) {
                // Add existing data point from the map
                result.push(dateMap.get(dateKey)!);
            } else if (!result.find(dp => dp.x === dateKey)) { // Avoid duplicates
                // For monthly data with future dates, y should be 0, otherwise carry forward the last y
                const y = monthly && currentDate > parseDateString(array[array.length - 1].x) ? 0 : (result.length > 0 ? result[result.length - 1].y : 0);
