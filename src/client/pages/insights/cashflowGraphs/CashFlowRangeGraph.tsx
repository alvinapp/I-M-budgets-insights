...
const CashFlowRangeGraph: React.FC<Props> = ({
    ...
    useEffect(() => {
        ...
        if (earnedData.length > 0 && spentData.length > 0 && fullDataLabels.length > 0) {
            setDataLoaded(true);
        }
    }, [earnedData, spentData, fullDataLabels]);

    // Convert Date objects to the expected string format
    const formattedDates = fullDataLabels?.map((date) =>
    ...
    const options: ApexOptions = {
        ...
        tooltip: {
            custom: ({ series, seriesIndex, dataPointIndex, w }) => {
                ...
                if (isSameMonth(new Date(w.config.series[0].data[0].x), new Date(w.config.series[0].data[w.config.series[0].data.length - 1].x))) {
                    date = format(new Date(date), 'MMM dd, yyyy');
                } else {
                    date = format(new Date(date), 'MMM, yyyy');
                }
                return (
                    `<div style="padding: 10px; background-color: #f4f9fb; border-radius: 8px;" class="custom-tooltip">
